  ...
  const { steps, currentStepIndex, step, isFirstStep, isLastStep, back, next, goTo } = useMultistepForm([
    ...data.map((item, index) =>
      Array.isArray(item.selected) ? (
        <CheckboxQuizPass key={index} {...item} updateFields={fields => updateFields(index, fields)} />
      ) : (
        <RadioQuizPass key={index} {...item} updateFields={fields => updateFields(index, fields)} />
      ),
    ),
  ]);
  return (
              ...
              <div
                ...
                className={clsx(styles.right__number, {
                  ...
                  [styles.right__number__passed]:
                    (!Array.isArray(INITIAL_DATA[index].selected) && data[index].selected !== "") || (Array.isArray(data[index].selected) && data[index].selected.length > 0),
                })}
                onClick={() => goTo(index)}
              >
                {index + 1}
